...
-- Indice per filtraggio per data range (rimosso per compatibilità)
-- CREATE INDEX IF NOT EXISTS idx_sessions_date_range 
-- ON sessions (session_date) WHERE session_date >= CURRENT_DATE - INTERVAL '30 days';
...
-- ON events (event_date) WHERE event_date >= CURRENT_DATE - INTERVAL '30 days';
...
-- ========================================
-- 17. CONFIGURAZIONI PERFORMANCE
-- ========================================

-- Configurazioni consigliate per Supabase (da applicare manualmente)
-- work_mem = 256MB
