-- ========================================
-- SCRIPT SICURO DI CORREZIONE DATABASE
-- ========================================
...
-- 2. CREA SOLO GLI ENUM MANCANTI (non toccare quelli esistenti)
DO $$ 
BEGIN
    -- Enum per luoghi infortunio (solo se non esiste)
    ...
    -- Enum per location sessioni (solo se non esiste)
...
-- 3. AGGIORNA SOLO LE COLONNE CHE NON HANNO GIÀ UN ENUM
...
-- Aggiorna sessions location se esiste e non è già del tipo corretto
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
