-- ========================================
-- SCRIPT CORRETTO DI CORREZIONE DATABASE
-- ========================================
...
-- Prima cambia i valori esistenti per renderli compatibili
UPDATE attendance SET status = 'PRESENTE' WHERE status = 'presente';
UPDATE attendance SET status = 'ASSENTE' WHERE status = 'assente';

-- Poi aggiorna il tipo della colonna
ALTER TABLE attendance 
ALTER COLUMN status TYPE attendance_status USING status::attendance_status;

-- Aggiorna injured_place se esiste
ALTER TABLE attendance 
ALTER COLUMN injured_place TYPE injury_place USING injured_place::injury_place;

-- Aggiorna sessions location
ALTER TABLE sessions 
ALTER COLUMN location TYPE session_location USING location::session_location;

-- Aggiorna profiles role
