-- ========================================
-- FIX RLS POLICIES PER ACCESSO COMPLETO A PEOPLE (CORRETTO)
-- ========================================
...
-- 3. Rimuovi TUTTE le policy esistenti su people
...
DROP POLICY IF EXISTS "Enable read access for all authenticated users" ON public.people;
DROP POLICY IF EXISTS "Enable insert for all authenticated users" ON public.people;
DROP POLICY IF EXISTS "Enable update for all authenticated users" ON public.people;
DROP POLICY IF EXISTS "Enable delete for all authenticated users" ON public.people;

-- 4. Crea policy per accesso COMPLETO a tutti gli utenti autenticati
...
-- 8. Verifica le nuove policy create
...
-- 9. Se people è ancora vuota, copia da people3
DO $$ 
BEGIN
