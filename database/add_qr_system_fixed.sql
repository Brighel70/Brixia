-- Sistema QR Code per Presenze - Script Corretto
...
-- 8. Aggiunge una vista per le statistiche QR in tempo reale (con valori enum corretti)
...
-- 12. Test: Inserisce una sessione di esempio per test (opzionale)
...
-- 13. Verifica che tutto sia stato creato correttamente
SELECT 
  'Sistema QR installato' as status,
  COUNT(*) as tables_updated
FROM information_schema.columns 
WHERE table_name IN ('sessions', 'attendance') 
  AND column_name IN ('qr_active', 'qr_code', 'scanned_at', 'qr_data');
