-- ========================================
-- SCRIPT FINALE DI CORREZIONE DATABASE
-- ========================================
...
-- 2. CREA GLI ENUM MANCANTI (con i valori esistenti)
DO $$ 
BEGIN
    -- Enum per status presenze (con valori esistenti)
...
-- Prima verifica se la colonna status usa già un enum
DO $$
BEGIN
    -- Se la colonna status non è già di tipo attendance_status, aggiornala
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'attendance' 
        AND column_name = 'status' 
        AND udt_name != 'attendance_status'
    ) THEN
        -- Aggiorna il tipo della colonna status
        ALTER TABLE attendance 
        ALTER COLUMN status TYPE attendance_status USING status::attendance_status;
    END IF;
END $$;

-- Aggiorna injured_place se esiste e non è già del tipo corretto
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'attendance' 
        AND column_name = 'injured_place' 
        AND udt_name != 'injury_place'
    ) THEN
        ALTER TABLE attendance 
...
-- Aggiorna sessions location se esiste
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- Aggiorna profiles role se esiste
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- 7. AGGIORNA I VALORI DI DEFAULT
...
-- 8. AGGIORNA I CHECK CONSTRAINTS
...
-- 9. AGGIORNA I VALORI DI DEFAULT MANCANTI
