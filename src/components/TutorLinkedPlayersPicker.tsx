...
export type TutorAthleteRelation = { athlete_id: string; relationship: string }

export type LinkedPlayerOption = {
  id: string
  name: string
...
interface TutorLinkedPlayersPickerProps {
  relations: TutorAthleteRelation[]
  optionsById: LinkedPlayerOption[]
  onAdd: (option: LinkedPlayerOption) => void
  onRemove: (athleteId: string) => void
  onRelationshipChange: (athleteId: string, relationship: string) => void
  disabled?: boolean
...
async function searchPlayersForLinking(
  query: string,
  excludeIds: Set<string>,
  limit = 40,
): Promise<LinkedPlayerOption[]> {
  const trimmed = query.trim()
  if (trimmed.length < MIN_CHARS) return []

  const [{ data: categoriesData }, { data: peopleData, error }] = await Promise.all([
  ...
  return (peopleData || [])
    ...
    .map((p) => {
      const categoryLabel = (p.player_categories || [])
        ...
        .filter(Boolean)
        .join(', ')
      const name = [p.family_name, p.given_name].filter(Boolean).join(' ')
        || p.full_name
...
export default function TutorLinkedPlayersPicker({
  relations,
  optionsById,
  onAdd,
  onRemove,
  ...
  const [results, setResults] = useState<LinkedPlayerOption[]>([])
  const [activeIndex, setActiveIndex] = useState(-1)

  const selectedIds = useMemo(() => relations.map((r) => r.athlete_id), [relations])
  ...
  const labelForId = (id: string): LinkedPlayerOption => {
    const opt = optionsById.find((o) => o.id === id)
    return opt || { id, name: 'Giocatore collegato' }
  }
  ...
  const runSearch = async (q: string) => {
    ...
    try {
      ...
      const rows = await searchPlayersForLinking(trimmed, exclude)
      setResults(rows)
  ...
  const pickOption = (opt: LinkedPlayerOption) => {
    onAdd(opt)
    setQuery('')
    setResults([])
    setActiveIndex(-1)
  ...
  const gridCols = disabled
    ...
    : 'grid-cols-[minmax(0,2fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_3.75rem]'
    ...
    <div>
      ...
      <div ref={containerRef} className="relative">
        <input
          value={query}
          disabled={disabled}
          placeholder="Cerca giocatore da collegare…"
          autoComplete="off"
          spellCheck={false}
        ...
        {showDropdown && (
          <div className="absolute bottom-full left-0 right-0 z-[100] mb-1 max-h-60 overflow-y-auto rounded-xl border-2 border-slate-200 bg-white shadow-[0_-8px_25px_rgba(0,0,0,0.12)]">
            {qLen < MIN_CHARS ? (
              ...
              <div className="px-4 py-3 text-sm text-slate-500">Nessun giocatore trovato</div>
            ) : (
              results.map((opt, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {(opt.categoryLabel || opt.age != null) && (
                      <span className={`mt-0.5 text-xs ${active ? 'text-blue-100' : 'text-slate-500'}`}>
                        {[opt.categoryLabel, opt.age != null ? `${opt.age} anni` : ''].filter(Boolean).join(' · ')}
                      </span>
                    )}
                  </button>
                )
      ...
      {relations.length > 0 ? (
        <div className={`${disabled ? 'mt-0' : 'mt-3'} overflow-hidden rounded-xl border border-slate-300 bg-white`}>
          <div className={`grid ${gridCols} items-center gap-x-4 border-b border-slate-200 bg-slate-100 px-3 py-2 text-[11px] font-bold uppercase tracking-[0.08em] text-[#334D70]`}>
            <span>Giocatore</span>
            <span>Categoria</span>
            <span className="text-center">Età</span>
            <span>Relazione</span>
            {!disabled && <span className="text-right">Azioni</span>}
          </div>
          <ul className="divide-y divide-slate-200">
            {relations.map((rel) => {
              const player = labelForId(rel.athlete_id)
              const category = player.categoryLabel || '—'
              const age = player.age != null ? `${player.age} anni` : '—'
              const isInvalid = invalidRelationIds.includes(rel.athlete_id)
              return (
                <li
                  key={rel.athlete_id}
                  className={`grid ${gridCols} items-center gap-x-4 px-3 py-2`}
                >
                  <span className="min-w-0 truncate text-sm font-bold text-emerald-900" title={player.name}>
                    {player.name}
                  </span>
                  <span className="min-w-0 truncate text-sm font-medium text-slate-800" title={category !== '—' ? category : undefined}>
                    {category}
                  </span>
                  <span className="whitespace-nowrap text-center text-sm font-medium text-slate-800">{age}</span>
                  {disabled ? (
                  ...
                  <select
                    ...
                    onChange={(e) => onRelationshipChange(rel.athlete_id, e.target.value)}
                  ...
                  <div className="flex justify-end">
                    <button
                      type="button"
                      onClick={() => onRemove(rel.athlete_id)}
        ...
        <p className="mt-2 text-xs text-slate-400">
          {disabled
            ? 'Nessun giocatore collegato a questo tutor.'
            : 'Nessun giocatore collegato. Cerca e seleziona uno o più giocatori a cui questo tutor fa riferimento.'}
        </p>
      )}
    </div>
  )
