...
export default function CategoryActivities({ embedInLayout = false }: CategoryActivitiesProps) {
  ...
  const handleEditEvent = (event: Event) => {
    ...
    setEventForm({
      ...
      match_result: (event as any).match_result || ''
    })
...
alert('Errore nell\'aggiornamento dell\'evento')
      ...
      // Chiudi modal e resetta form (nessun popup in caso di successo)
...
{/* Modal per le presenze */}
      ...
      {showEditEventModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-[2px]">
          <div className="max-h-[90vh] w-full max-w-4xl overflow-auto rounded-[28px] border border-slate-200 bg-white shadow-[0_28px_80px_rgba(15,23,42,0.28)]">
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-5 sm:px-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-black text-slate-950">Modifica evento</h2>
                  <p className="mt-0.5 text-sm text-slate-500">Aggiorna date, luogo e dettagli dell&apos;attività.</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="grid grid-cols-1 gap-x-5 gap-y-4 md:grid-cols-2 mb-5">
              {/* Colonna Sinistra */}
              <div className="space-y-4">
                {/* Tipo Evento */}
                <div>
                  <label className="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-600">Tipo evento <span className="text-rose-500">*</span></label>
                  <select
                    value={eventForm.event_type}
                    onChange={(e) => setEventForm(prev => ({ ...prev, event_type: e.target.value }))}
                ...
                {/* Categoria */}
                <div>
                  <label className="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-600">Categoria</label>
                  <select
                    value={eventForm.category_id}
                    onChange={(e) => setEventForm(prev => ({ ...prev, category_id: e.target.value }))}
                    className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm font-medium text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10"
                ...
                {/* Data Evento */}
                <div>
                  <label className="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-600">Data evento <span className="text-rose-500">*</span></label>
                  <input
                    type="date"
                    value={eventForm.event_date}
                    onChange={(e) => setEventForm(prev => ({ ...prev, event_date: e.target.value }))}
              ...
              <div className="space-y-4">
                {/* Titolo Evento */}
                <div>
                  <label className="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-600">Titolo evento <span className="text-rose-500">*</span></label>
                  <input
                    type="text"
                    value={eventForm.title}
                    onChange={(e) => setEventForm(prev => ({ ...prev, title: e.target.value }))}
                    placeholder="Es. Partita U14 vs Rugby Milano"
                    className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm font-medium text-slate-800 placeholder:text-slate-400 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10"
                  />
                </div>

                {/* Avversario */}
                <div>
                  <label className="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-600">Avversario <span className="font-medium normal-case text-slate-400">(per partite)</span></label>
                  <input
                    type="text"
                    value={eventForm.opponent}
                    onChange={(e) => handleOpponentChange(e.target.value)}
            ...
            <div className="grid grid-cols-1 gap-x-5 gap-y-4 md:grid-cols-2 mb-5">
              <div>
                <label className="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-600">Location</label>
                <TrainingVenueSelect
                  value={eventForm.location}
                  onChange={handleLocationChange}
                  className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm font-medium text-slate-800 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10"
              ...
              <div>
                <label className="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-600">Dove giocare</label>
                <input
                  type="text"
                  value={eventForm.away_location}
                  onChange={(e) => setEventForm(prev => ({ ...prev, away_location: e.target.value }))}
            ...
            {/* Ora Inizio e Ora Fine (visibili quando è selezionata una location) */}
            {eventForm.location && (
              <div className="mb-5 grid grid-cols-1 gap-x-5 gap-y-4 md:grid-cols-2">
                <div>
                  <label className="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-600">Ora inizio</label>
                  <input
                    type="time"
                    value={eventForm.start_time || ''}
                    onChange={(e) => setEventForm(prev => ({ ...prev, start_time: e.target.value }))}
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-600">Ora fine</label>
                  <input
                    type="time"
                    value={eventForm.end_time || ''}
                    onChange={(e) => setEventForm(prev => ({ ...prev, end_time: e.target.value }))}
            ...
            {/* Checkboxes */}
            <div className="mb-5 flex flex-wrap gap-x-6 gap-y-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5">
              <label className="flex items-center cursor-pointer">
                ...
                />
                <span className="text-sm font-medium text-slate-700">In trasferta</span>
              </label>
              <label className="flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  ...
                  className="mr-2 h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                />
                <span className="text-sm font-medium text-slate-700">Campionato</span>
              </label>
              <label className="flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  ...
                  className="mr-2 h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                />
                <span className="text-sm font-medium text-slate-700">Amichevole</span>
              </label>
            </div>

            {/* Descrizione */}
            <div className="mb-6">
              <label className="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-600">Descrizione</label>
              <textarea
                value={eventForm.description}
                onChange={(e) => setEventForm(prev => ({ ...prev, description: e.target.value }))}
                placeholder="Descrizione dell'evento..."
                rows={3}
                className="w-full resize-y rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-800 placeholder:text-slate-400 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10"
              />
            </div>
            ...
            <div className="flex flex-col-reverse gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:justify-end">
              <button
                onClick={closeEditEventModal}
                className="rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-700 transition-colors hover:bg-slate-50 sm:min-w-36"
              >
                Annulla
              </button>
              <button
                onClick={handleUpdateEvent}
                className="rounded-xl bg-blue-600 px-5 py-3 text-sm font-bold text-white shadow-sm transition-colors hover:bg-blue-700 sm:min-w-44"
              >
                Aggiorna Evento
              </button>
            </div>
